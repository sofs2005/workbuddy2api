...
// SetRealm 显式设置持久化 realm 标识（登录落盘时由调用方按 --realm/domain 归一后写入）。
// 显式非空优先，空值按 domain 推断（ResolveRealm 口径）——保证落盘凭证恒带 realm 键。
// 与 BackfillRealm 的区别：Backfill 幂等（已有标识不动），SetRealm 无条件覆盖（登录是权威来源，
// 用户显式 --realm=cn 应能改写此前按 domain 推断出的 global）。
func (a *Auth) SetRealm(realm string) { a.realm = ResolveRealm(realm, a.Domain) }
