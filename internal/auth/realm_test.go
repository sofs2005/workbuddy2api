...
// TestSetRealmOverridesBackfill SetRealm 相对 BackfillRealm 的关键差异：无条件覆盖。
// 登录是 realm 的权威来源，用户显式 --realm=cn 应能改写此前按 domain 推断出的 global
// （BackfillRealm 幂等，已有标识不动，做不到这件事）。
func TestSetRealmOverridesBackfill(t *testing.T) {
	t.Parallel() // 纯函数：不触碰全局开关
	// 已有 realm=global、domain 为 CN：显式传 cn 必须改写为 cn
	a := &Auth{Domain: "www.codebuddy.cn", realm: "global"}
	a.SetRealm("cn")
	assertRealmStored(a, "cn", t)

	// 空显式值 → 按 domain 推断（与 ResolveRealm 同口径）
	b := &Auth{Domain: "www.workbuddy.ai"}
	b.SetRealm("")
	assertRealmStored(b, "global", t)

	// 显式值优先于 domain（global domain + 显式 cn → cn）
	c := &Auth{Domain: "www.workbuddy.ai"}
	c.SetRealm("cn")
	assertRealmStored(c, "cn", t)
}

// TestSetRealmPersists SetRealm 的结果经 SaveAtomic 落盘后，Parse 回读一致
// （登录落盘链路的持久化保证）。
func TestSetRealmPersists(t *testing.T) {
	dir := t.TempDir()
	file := filepath.Join(dir, "workbuddy-u9.json")
	a := &Auth{AccessToken: "at", UID: "u9", Domain: "www.codebuddy.cn", FilePath: file}
	a.SetRealm("global") // 显式 global 覆盖 CN domain 推断
	if err := a.SaveAtomic(); err != nil {
	...
	if got := parsed.RealmStored(); got != "global" {
		t.Errorf("persisted realm=%q want global", got)
	}
}
